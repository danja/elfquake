...
* Damage-enabled dynamics are the first supported synthetic precursor candidate. Pre-relaxation `damage_total` has a 5--15 step causal lead across nine episodes (AUC `0.652315`, 6/9 positive episode effects). Treat it as a separate synthetic modality and require leave-one-episode-out improvement against damage-disabled data before promotion to the default Transformer configuration.
