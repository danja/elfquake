...
The 2026-07-29 domain probe used the refreshed 257-window Cumiana sequence and three 3,000-step synthetic piezo sequences. The closest 25% synthetic windows reduced centroid distance from `3.045` to `2.531` and nearest-real distance from `2.186` to `1.404`, but synthetic reconstruction MSE remained `11.604` versus `0.562` for real held-out windows. Inlier selection is therefore a useful diagnostic, not evidence that synthetic pretraining is aligned; improve simulator shape statistics before another transfer sweep.

The four-profile 20,000-step shape sweep is stored at `data/derived/models/piezo_vlf_alignment_sweep/piezo_vlf_alignment_sweep.csv`. Its simulation-seed follow-up is `data/derived/models/piezo_vlf_alignment_seed_holdout/summary.csv`. An independent alignment-model-seed check is `data/derived/models/piezo_vlf_alignment_model_seed_holdout/summary.csv`. Across simulation seeds and model seeds, `gain_burst` has the best mean centroid distance (`1.7488`) and nearest-real distance (`1.0537`); `fast_burst` is close (`1.7558` and `1.0636`), while current has a similar centroid (`1.7490`) but weaker nearest distance (`1.2575`). The wide per-run ranges make this a weak preference only; retain `gain_burst` as a candidate rather than changing the simulator default.
