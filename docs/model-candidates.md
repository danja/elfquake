...
The four-profile 20,000-step shape sweep is stored at `data/derived/models/piezo_vlf_alignment_sweep/piezo_vlf_alignment_sweep.csv`. Its multi-seed follow-up is `data/derived/models/piezo_vlf_alignment_seed_holdout/summary.csv`. Across simulation seeds `40`, `41`, and `42`, `gain_burst` has the best mean centroid distance (`1.6595`) and nearest-real distance (`0.8467`); `fast_burst` is close (`1.6617` and `0.8565`), while current is weaker on nearest distance (`1.2872`). The ordering is narrow and the alignment model seed is fixed, so retain `gain_burst` as a candidate rather than changing the simulator default.
