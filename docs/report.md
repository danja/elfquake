...
The first multi-episode candidate is now available in `data/derived/reports/central_italy_event_catalog_alignment_combined_spatial.json`. Three independent episodes were offset by 21 days, rate-thinned, magnitude-calibrated, and then spatially transported as an explicit observation model. It reaches rate ratio `0.956`, magnitude Wasserstein distance `0.082`, and median inter-event time `23.6` hours versus `21.6` hours for real central Italy. Spatial quantile transport reduces nearest-neighbour Wasserstein distance from `30.939` to `11.212` km and covers 3 of 4 coarse cells. This is the first promising intermediate alignment, but not yet a satisfactory match for spatial clustering.
