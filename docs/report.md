...
The latest reproducible coverage report combines `4,836` normalized INGV events from 2024-01-01 through 2026-07-07 with `283` Cumiana capture metadata records through 2026-07-16. The current self-supervised VLF series contains `256` anomaly windows through 2026-07-16. Only two calendar weeks currently contain both VLF observations and seismic events, so the VLF/seismic overlap is still too short for a meaningful supervised comparison. The anomaly series has `37` scores at or above the exploratory `0.8` threshold, but these are label-free novelty flags and have not been shown to correspond to earthquakes. The machine-readable outputs are `data/derived/reports/italy_data_coverage.json` and `data/derived/reports/italy_data_coverage_weekly.csv`.

A permutation-controlled association check gives the same conclusion. There are now three VLF-observed weeks, but at M2.5+ and M3+ only one is an event week and two are controls; at M2+ there are two event weeks and one control. The report therefore still returns `insufficient_controls` rather than a p-value. The apparent lower anomaly score in event weeks is not evidence of a precursor. Outputs are `data/derived/reports/italy_vlf_event_association.json` and `data/derived/reports/italy_vlf_event_association_weekly.csv`.

### Label-scope audit

The apparently mirrored all-Italy/central-Italy counts are not a row-generation error. Both tables are anchored on the same 277 mature Cumiana windows, so equal row counts are expected. The current target definition is saturated: every mature all-Italy seven-day window contains at least one M3+ event (`277/277` positive), while every mature central-Italy window contains none (`0/277`). At M2.5+, central Italy becomes usable for a limited exploratory split (`228/49` positive/negative), but all-Italy remains saturated (`277/0`). The all-Italy table must therefore move to fixed spatial-cell targets or event-count regression; it must not be used as a binary supervised table in its current form.
