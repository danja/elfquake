...
The new per-episode diagnostic shows why the combined result must remain provisional. With explicit episode durations but before calibration, seeds `40`--`42` produce rate ratios of `14.435`, `14.324`, and `14.990`, while seeds `4300` and `4500` produce `0.555` and `0.278`. The simulation therefore has distinct event-rate regimes under nominally similar settings. Global thinning can make the combined rate look realistic, but it cannot by itself show that the underlying event process is realistic or transferable. Reports are stored under `data/derived/reports/central_italy_episode_alignment/`.
