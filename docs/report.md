...
The combined calibration is available through `./scripts/calibrate-italy-synthetic-catalog.sh`. For seed `40`, deterministic rate thinning reduced the rate ratio from `1.998` to `1.143`; magnitude Wasserstein distance fell from `2.316` to `0.146`, and the magnitude KS statistic fell from `0.999` to `0.281`. Spatial support remained incomplete, falling from 13 to 11 occupied cells versus 63 in the real catalog. Rate and magnitude alignment are therefore promising preprocessing checks, not evidence that the simulated process is physically aligned.
