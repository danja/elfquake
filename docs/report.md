...
The first reproducible coverage report combines `4,836` normalized INGV events from 2024-01-01 through 2026-07-07 with `282` Cumiana capture metadata records across nine days. The current self-supervised VLF series contains `224` anomaly windows from 2026-06-30 through 2026-07-06. Only two calendar weeks currently contain both VLF observations and seismic events, so the VLF/seismic overlap is still too short for a meaningful supervised comparison. The anomaly series has `33` scores at or above the exploratory `0.8` threshold, but these are label-free novelty flags and have not been shown to correspond to earthquakes. The machine-readable outputs are `data/derived/reports/italy_data_coverage.json` and `data/derived/reports/italy_data_coverage_weekly.csv`.

A permutation-controlled association check gives the same conclusion. At M2.5+ and M3+, the two observed VLF weeks split into one event week and one control week; at M2+ both weeks contain events. The report therefore returns `insufficient_controls` rather than a p-value. The apparent M2.5+ anomaly-score difference is not evidence of a precursor. Outputs are `data/derived/reports/italy_vlf_event_association.json` and `data/derived/reports/italy_vlf_event_association_weekly.csv`.
