...
* `sim.sh` now exposes `DEPOSITION_PROBABILITY`, and `run-synthetic-episode-batch.sh` now defaults to a more aggressive stationarity profile: target fill limit `WIDTH * HEIGHT / 128`, bottom-layer removal every `25` steps, and deposition probability `0.45`.
...
* Interpretation: initial fill can skip cold-start height growth, but with immediate bottom-layer removal and the current event-extraction settings it does not by itself produce a more stationary event process. Keep the aggressive no-prefill 3000-step profile as the current best stationarity baseline, and revisit initial fill only with delayed removal or a short unrecorded warm-up.
