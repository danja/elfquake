...
Two research threads have grown substantially since the sections above were written and are not yet reflected there.

**Japan VLF cross-region thread.** A parallel, research-use-only pipeline ingests ISEE/ERGSC Moshiri (and, as a fallback, Kagoshima) VLF CDF archives alongside USGS Japan seismic history, mirroring the Italy pipeline's structure but kept strictly separate and scoped outside the Italy project boundary. As of 2026-08-01 the combined Japan seismic catalog covers 1,353 normalized events and 133 complete weekly M5 windows, with 9 VLF-observed windows against 124 missing; the comparable M5.0 tabular smoke model scored `0.375` calibrated balanced accuracy on 8 observed-VLF windows and is not usable evidence. A dedicated check around the July 2026 Moshiri-region earthquake sequence found the pre-event VLF anomaly score elevated on several days across 15–29 July (`2.322`–`8.772`), but elevation appears on multiple days both before and after the event, and archive coverage for 28–30 July was incomplete at check time, so this is a candidate signal only, not event-specific precursor evidence. A mixed-source alignment audit (Italy + Japan + synthetic) found 5,301 Italy seismic/VLF/astronomy rows, 8 Japan seismic/observed-VLF rows, and 167 fully co-observed synthetic rows; a 48-manifest cross-source patch-Transformer smoke run completed end-to-end but scored `0.489320` calibrated balanced accuracy, below the majority baseline, so it remains an interface/masking check only.

A follow-on Cross-Region Generative Smoke Test chained synthetic masked pretraining, Japan self-supervised continuation, and chronological Italy fine-tuning (seismic + VLF + astronomy) into one continuous coordinate/magnitude event-list output. **Re-run 2026-08-22 against the live catalog, and the first run in which the astronomy modality carried real channels** — until that date the sequence tensors the model reads lagged the feature table and still held the two dead channels described in [Astronomy Alignment](astronomy-alignment.md). The CPU run used 11,004 training and 2,752 test Italy rows with 678 test positives, trained on 45 Japan windows, and emitted 5 continuous event rows against 13 actual events in the held-out week. Held-out balanced accuracy was `0.513960` with all three modalities, `0.511239` with astronomy masked, `0.511085` with seismic masked, `0.493117` with Italy VLF masked, and `0.498439` for a linear probe on the frozen representation — all indistinguishable from `0.5` on one seed, and masking measures inference-time reliance rather than modality value. This confirms the cross-region interface works end to end; it demonstrates no prediction skill, and Japan data remains restricted to research use. The earlier run on this text used 5,263 mature Italy rows and held out 1,023.

**Updated matched ablation suite.** The weekly fixed-cell evaluation (seismic-history-only, VLF-mask-only, full multimodal, synthetic-pretrained) was rerun across seeds `42`, `99`, and `123`. Full/pretrained averaged `0.690230` balanced accuracy (`0.684675`–`0.693435`) versus `0.671673` without pretraining; seismic-history/pretrained averaged `0.685992`, and seismic+VLF-mask/pretrained `0.689776`. VLF-mask-only stayed near `0.601`. Because the holdout period's VLF mask is constant across cells, this ablation cannot by itself identify VLF utility, and no modality-value claim should be drawn from these numbers.

Neither thread changes the report's bottom-line conclusion: useful strong-earthquake prediction is not currently demonstrated or operationally viable.
