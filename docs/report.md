...
A follow-on Cross-Region Generative Smoke Test chained synthetic masked pretraining, Japan self-supervised continuation, and chronological Italy fine-tuning (seismic + VLF + astronomy) into one continuous coordinate/magnitude event-list output. The CPU run used 5,263 mature Italy rows, held out 1,023, trained on 45 Japan windows, and emitted 5 continuous event rows. This confirms the cross-region interface works end to end; it demonstrates no prediction skill, and Japan data remains restricted to research use.
