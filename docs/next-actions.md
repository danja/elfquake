...
43. Use `data/derived/models/japan_vlf_model_input.m5.csv` as the current Japan smoke-training table only. Its M5.0 target has `49/29` positive/negative windows; five windows now contain observed VLF, including three later/test-era windows. The first 80/20 tabular run did not beat the majority baseline, and coverage remains too sparse for tuning.
...
46. Require more than one observed Japan VLF capture in both chronological train and test periods before evaluating the Japan Transformer. The absolute coverage blocker is cleared, but five windows remain insufficient for a reliable score and most target rows are still masked.
