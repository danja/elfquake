...
43. Use `data/derived/models/japan_vlf_model_input.m5.csv` as the current Japan smoke-training table only. Its comparable M5.0 target has `49/29` positive/negative windows; seven now contain observed VLF, including five later/test-era windows, but all observed windows are positive. The first 80/20 tabular run did not beat the majority baseline, and coverage remains too sparse for tuning.
...
50. Run `./scripts/audit-common-transformer-alignment.sh` before any mixed-source model comparison. The current audit finds 5,301 Italy seismic/VLF/astronomy rows, 3 Japan seismic/observed-VLF rows, and 167 fully co-observed synthetic sensor rows; acquire more temporally matched Japan VLF before evaluating Japan transfer.
