...
46. Require at least one observed Japan VLF capture in both chronological train and test periods before evaluating the Japan Transformer. The current capture-safe run correctly reports `insufficient_sequence_covered_rows` because all three observed windows are in train.
