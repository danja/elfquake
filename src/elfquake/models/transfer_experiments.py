...
def run_transfer_experiment_suite(
    *,
    ...
    torch = _torch()
    ablations = {
        "historical_spatial_rate": _rate_result(train, test),
        "real_random_init_seismic": _model_result(torch, train, test, synthetic_samples=None, epochs=epochs, pretrain_epochs=0, seed=seed),
        "synthetic_pretrained_transfer": _model_result(torch, train, test, synthetic_samples=synthetic_samples, epochs=epochs, pretrain_epochs=pretrain_epochs, seed=seed),
    }
    rolling = _rolling_results(torch, samples, synthetic_samples, weeks, epochs, pretrain_epochs, seed)
    grid = _grid_selection(torch, real, synthetic, train_weeks, test, magnitude_threshold, horizon_days, epochs, pretrain_epochs, seed)
    report = {
...
def _model_result(torch, train, test, *, synthetic_samples, epochs, pretrain_epochs, seed):
    ...
    return {"status": "evaluated", "training": "synthetic_pretrained" if transfer else "real_random_initialization", "threshold": round(threshold, 6), **_metrics(test_probabilities, [sample.label for sample in test], threshold)}


def _rate_result(train, test):
    train_probabilities = _historical_cell_rates(train, train)
    test_probabilities = _historical_cell_rates(train, test)
    threshold = _best_threshold(train_probabilities, [sample.label for sample in train])
    return {"status": "evaluated", "threshold": round(threshold, 6), **_metrics(test_probabilities, [sample.label for sample in test], threshold)}


def _rolling_results(torch, samples, synthetic_samples, weeks, epochs, pretrain_epochs, seed):
    results = []
    for index, fraction in enumerate((0.60, 0.70, 0.80, 0.85)):
        ...
        train = [sample for sample in samples if sample.week_start in train_weeks]
        test = [sample for sample in samples if sample.week_start in test_weeks]
        result = _model_result(torch, train, test, synthetic_samples=synthetic_samples, epochs=epochs, pretrain_epochs=pretrain_epochs, seed=seed + index)
...
def _grid_selection(torch, real, synthetic, train_weeks, final_test, default_threshold, horizon_days, epochs, pretrain_epochs, seed):
    candidates = []
    ...
    for threshold in (2.5, 2.7, 3.0):
        for size in (1.0, 1.5, 2.0):
            ...
            if len({sample.label for sample in train}) < 2 or len({sample.label for sample in validation}) < 2:
                continue
            result = _model_result(torch, train, validation, synthetic_samples=None, epochs=max(15, epochs // 2), pretrain_epochs=0, seed=seed)
    ...
    if selected:
        ...
        final_result = _model_result(torch, selected_train, selected_test, synthetic_samples=selected_synthetic, epochs=epochs, pretrain_epochs=pretrain_epochs, seed=seed + 100)
