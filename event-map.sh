...
Defaults:
  events_csv     newest combined normalized INGV CSV, else newest synthetic event CSV
...
if [[ -z "$input" || ! -f "$input" ]]; then
  echo "error: no event CSV found. Pass a normalized or synthetic event CSV explicitly." >&2
  exit 2
fi
