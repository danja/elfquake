...
ROOT="${ROOT:-data/derived/models/self_supervised_transformer}"
...
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src "$PYTHON_BIN" -m elfquake.cli evaluate-self-supervised-transformer \
  ...
  "${manifest_args[@]}" \
  --real-sequence-manifest "${REAL_SEQUENCE:-data/derived/models/cumiana_vlf_image_sequence/manifest.json}" \
  --out "$OUT" \
  ...
  --max-pretrain-windows "${MAX_PRETRAIN_WINDOWS:-4096}"
